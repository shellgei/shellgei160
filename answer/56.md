...
### 別解
```
別解1（上田）$ sed -E 's;^(.*):(.*);s/\1/\2/g;g' service_stop_weekday.txt | sed -f - service_depend_list.txt | tr -d :, | sed 's/  */ /g' | tarr num=1 | self 2 1 | sort -u | yarr num=1 | sed 'y/日月火水木金土/1234567/' | sort | sed 'y/1234567/日月火水木金土/' | sed 's/ /, /g;s/,/:/'
