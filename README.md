...
|-----|--------------------|-------------|----------------|----------|----------|
|p.61 |練習2.1.e 2段落目の最後の2文|ダブルクォートによるクォートの場合、中にあるBashの変数は解釈されます。また、変数だけでなく、ホームディレクトリを表す˜などの特殊な記号も解釈されます。|ダブルクォートによるクォートの場合、変数やコマンド置換（`$`のつくもの）は解釈されます。|[issue50](https://github.com/shellgei/shellgei160/issues/50)|完全に間違いです。勘違いをそのまま確認せずに書き流してしまいました。すみません:pray:|
### 第2刷まで
